...
export const updatePerfil = async (req, res) => {
  try {
    const { id } = req.params;
    const { nombre_completo, email, telefono, cargo } = req.body;
 
    // Si Multer capturó una imagen, construimos la ruta
    let avatar_url = null;
    if (req.file) {
      avatar_url = `/uploads/profiles/${req.file.filename}`;
    }
 
    // COALESCE: si avatar_url es NULL, deja el que ya tenía en la BD
    const { rows } = await pool.query(
      `UPDATE users 
